...
use Proximum\Vimeet365\Application\View\TimezoneView;
...
final class TimezoneDataProvider implements ItemDataProviderInterface, CollectionDataProviderInterface, RestrictedDataProviderInterface
{
    ...
    {
        $timezones = array_map(fn (string $code) => new TimezoneView($code, Timezones::getName($code)), Timezones::getIds());

        usort($timezones, fn (TimezoneView $a, TimezoneView $b): int => $a->name <=> $b->name);
