...
use Proximum\Vimeet365\Application\View\LanguageView;
...
final class LanguageDataProvider implements ItemDataProviderInterface, CollectionDataProviderInterface, RestrictedDataProviderInterface
{
    ...
    public function getCollection(string $resourceClass, ?string $operationName = null): array
    {
        ...
        $languages = array_map(fn (string $code) => new LanguageView($code, Languages::getName($code)), $languages);

        usort($languages, fn (LanguageView $a, LanguageView $b): int => $a->name <=> $b->name);
